document.addEventListener("DOMContentLoaded", function() {
  ...
  for (var i = 0, len = tabs.length; i < len; i++) {
    tabs[i].addEventListener("click", function() {
      if (this.classList.contains('tabbed_active'))
        return;
      ...
      for (var index = 0, iLen = innerTabs.length; index < iLen; index++) {
        innerTabs[index].classList.remove('tabbed_active');
      }

      this.classList.add('tabbed_active');
    });
  }
  ...
  for (var i = 0, len = switchers.length; i < len; i++) {
    switchers[i].addEventListener("click", function() {
      ...
      for (var index = 0, iLen = innerSwitchers.length; index < iLen; index++) {
        innerSwitchers[index].classList.remove('tabbed_active');
      }
